...
const char* topicWebThresholds = "iot/web/thresholds";
...
void reconnectMQTT(){
  while (!mqttClient.connected()) {
    ...
    if (mqttClient.connect(clientId.c_str(), mqttUser, mqttPassword)) {
      ...
      mqttClient.subscribe(topicWebThresholds);
    } else {
      delay(5000);
    }
  }
...
void mqttCallback(char* topic, byte* payload, unsigned int length) {
  ...
  } else if (String(topic) == topicWebThresholds) {
    // Xử lý ngưỡng từ web
    StaticJsonDocument<200> doc;
    DeserializationError error = deserializeJson(doc, message);
    if (!error) {
      if (doc.containsKey("setup_soil")) {
...
void sendDataMQTT() {
  ...
  doc["setup_soil"] = setupSoilMoisteur;
  doc["setup_light"] = setupLightSensor;
